...
void LLFileSystem::updateFileAccessTime(const std::string& file_path)
{
    ...
    const std::time_t last_write_time = boost::filesystem::last_write_time(ll_convert_string_to_wide(file_path), ec);
    if (ec.failed())
    {
    ...
    {
        boost::filesystem::last_write_time(ll_convert_string_to_wide(file_path), cur_time, ec);
    }
#else
    // file last write time
