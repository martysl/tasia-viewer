	...
	mAvatarNameCacheConnection()
{
	...
	{
		mVoiceChannel  = new LLVoiceChannelP2P(session_id, name, other_participant_id, LLVoiceClient::getInstance()->getOutgoingCallInterface());
	}
	else
	{
...
LLIMModel::LLIMSession::~LLIMSession()
{
	...
	if(LLVoiceClient::getInstance() && mOtherParticipantID.notNull())
	{
		...
		case IM_SESSION_P2P_INVITE:
			// No equivalent in new voice API
			break;
...
void LLIncomingCallDialog::processCallResponse(S32 response, const LLSD &payload)
{
	...
	case 0: // accept
	{
		if (type == IM_SESSION_P2P_INVITE)
		{
			...
			LLSD voice_channel_info;
			voice_channel_info["session_handle"] = payload["session_handle"];
			voice_channel_info["caller_uri"] = payload["session_uri"];
