...
// For Linux/OSX the old routinewith std::stringstream should be fine. Those systems use a UTF8 codepage.
    ...
    std::wstring wDecimals = ll_convert_string_to_wide(decimals);
    std::wstring wNumStr = ll_convert_string_to_wide(numStr);
    ...
    if (!intDecimals)
    {
        ...
        {
            strStream << intStr;
            numStr = ll_convert_wide_to_string( strStream.str() );
        }
    }
    else
    {
