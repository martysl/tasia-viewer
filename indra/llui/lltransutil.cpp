...
bool LLTransUtil::parseStrings(const std::string& xml_filename, const std::set<std::string>& default_args)
{
    ...
    if (!success)
    {
        ...
        LL_ERRS() << "Couldn't load string table " << xml_filename << ". Please reinstall viewer from https://github.com/martysl/tasia-viewer/releases and contact https://support.easierit.org if issue persists after reinstall." << LL_ENDL;
        return false;
    }
